"""JAX implementation of information sensitivity metrics."""

from typing import Callable, Iterable, Optional

from .hessians import _DEFAULT_APPROXIMATOR, get_approximator
...
def reciprocal(f: Callable[[Any], ArrayLike]) -> Callable[[Any], Array]:
    ...
    def _inverted(*args, **kwargs) -> Array:
        return 1. / f(*args, **kwargs)

    return _inverted


def parameter_gradient(f: StatelessModel) -> Callable[[ArrayLike, ArrayLike], Array]:
    ...
    @grad
    def _f(params: ArrayLike, 
           x: ArrayLike) -> Array:
        return jnp.sum(f(x, *params))

    return _model


def parameter_hessian_diagonal(f: StatelessModel, 
                               approximator: str = _DEFAULT_APPROXIMATOR, 
                               *args, **kwargs) -> Callable[[ArrayLike, ArrayLike], Array]:
    ...
    @partial(vmap, in_axes=(None, 0))
    @partial(get_approximator(approximator), *args, **kwargs)
    def _f(params: ArrayLike, 
           x: ArrayLike) -> Array:
        return jnp.sum(f(x, *params))
...
def _model_loss(model: StatelessModel, 
                loss: LossFunction) -> Callable[[ArrayLike, ArrayLike, ArrayLike], float]:
...
def fisher_score(model: StatelessModel, 
                 loss: LossFunction) -> Callable[[ArrayLike, ArrayLike, ArrayLike], Array]:
    ...
    >>> mse_fn = lambda ypred, ytrue: jnp.sum(jnp.square(ypred - ytrue))
    >>> x = jnp.array([1., 2.])
    >>> p = [0., 2.]
    >>> model(x, *p)
    Array([2., 5.])
...
def fisher_information_diagonal(model: StatelessModel, 
                                loss: LossFunction, 
                                approximator: str = _DEFAULT_APPROXIMATOR,
                                *args, **kwargs) -> Callable[[ArrayLike, ArrayLike, ArrayLike, ArrayLike], Array]:
    ...
    return get_approximator(approximator)(_model_loss(model, loss), 
                                          *args, **kwargs)


def doubtscore(model: StatelessModel, 
               loss: LossFunction, 
               reciprocal: bool = False) -> Callable[[ArrayLike, ArrayLike, ArrayLike, ArrayLike], Array]:
...
def _information_sensitivity_term1(model: StatelessModel, 
                                  loss: LossFunction, 
                                  approximator: str = _DEFAULT_APPROXIMATOR,
                                  *args, **kwargs) -> Callable[[ArrayLike, ArrayLike, ArrayLike, ArrayLike], Array]:
    param_grad_fn = parameter_gradient(model)
...
def _information_sensitivity_term2(model: StatelessModel, 
                                  loss: LossFunction, 
                                  approximator: str = _DEFAULT_APPROXIMATOR,
                                  *args, **kwargs) -> Callable[[ArrayLike, ArrayLike, ArrayLike, ArrayLike], Array]:
    fisher_score_fn = fisher_score(model, loss)
    ...
    def _term2(params: ArrayLike, x: ArrayLike, 
               x_true: ArrayLike, y_true: ArrayLike) -> Array:
        return (fisher_score_fn(params, x_true, y_true) * param_hessian_fn(params, x)
                / jnp.square(param_grad_fn(params, x)))

    return _term2


def information_sensitivity(model: StatelessModel, 
                            loss: LossFunction, 
                            approximator: str = _DEFAULT_APPROXIMATOR,
    ...
    term1_fn, term1_fn = (f(model, loss, approximator, *args, **kwargs)
                          for f in (_information_sensitivity_term1,
                                    _information_sensitivity_term2))

    def _information_sensitivity(params: ArrayLike, x: ArrayLike, 
                                 x_true: ArrayLike, y_true: ArrayLike) -> Array:
