...
def mse_loss(y_pred: ArrayLike, y_true: ArrayLike) -> Array:
...
class DoubtMixin(DoubtMixinBase):
    ...
    def release_memory() -> None:
        """For compatibility. Doesn't do anything.
        """
        return None
    ...
    def _pack_params_like(
        ...
        for name, param in d.items():
            ...
            output[name] = jnp.reshape(p[index:end_index], (-1, *param.shape))
    ...
    #         return gradient_fn((params,), x_true, y_true)[0]
    ...
    def doubtscore_core(
        ...
        doubtscore = parameter_gradient / (fisher_score[jnp.newaxis] + eps)
        return doubtscore.flatten(start_dim=1, end_dim=-1)
    ...
    def information_sensitivity_core(
        ...
        term1 = fisher_information_diagonal[jnp.newaxis] * parameter_gradient
        if not (optimality_approximation or parameter_hessian_diagonal is None):
            term2 = fisher_score[jnp.newaxis] * parameter_hessian_diagonal
        else:
            term2 = 0.
        information_sensitivity = jnp.square(parameter_gradient) / (term1 - term2 + eps)
        return information_sensitivity.flatten(start_dim=1, end_dim=-1)
