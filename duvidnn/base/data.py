    ...
    def load_training_data(
        ...
        **preprocessing_args
        ...
        if len(input_towers) == 1:
            self.input_shape = np.asarray(self.training_example[input_towers[0]]).shape[1:]
        else:
            input_shape = tuple(
                np.asarray(self.training_example[key]).shape[1:] for key in input_towers
            )
        ...
        self.output_shape = np.asarray(self.training_example[self._out_key]).shape[1:]
        return None
